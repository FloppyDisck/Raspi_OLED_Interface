...
void GUI_LaunchScreen() {
	for (int i=0; i < 5; i++) {
        ...
        GUI_DisString_EN(1, (OLED_Y_MAXPIXEL/2) - 6, "Hola! Soy RPi!", &Font12, FONT_BACKGROUND, WHITE);

        if (i % 2 == 0) 
            GUI_Disbitmap(OLED_X_MAXPIXEL - 24, (OLED_Y_MAXPIXEL/2) - 10, Hand, 16, 20);
        else
            GUI_Disbitmap(OLED_X_MAXPIXEL - 26, (OLED_Y_MAXPIXEL/2) - 10, Hand_Angle, 24, 20);
